 ...
 */
export const getRecommendedRecipe = async (category = '전체') => {
  // 1. API → 여러 개 가져오기 (e.g., 100 items)
  // We use a larger pool to increase chances of finding preferred categories
  const randomStart = Math.floor(Math.random() * 800) + 1;
  const recipes = await fetchRecipes(randomStart, randomStart + 199); // Fetch 200 items

  if (recipes.length === 0) return null;

  // 2. 필터링 (기본 필터링: 이름 유무 등)
  let baseFiltered = recipes.filter(r => r.name && r.name.length > 1);
  ...
  if (category !== '전체') {
    ...
    // 해당 카테고리에 맞는 음식이 있다면 그것들 중에서만 선택
    if (categorySpecific.length > 0) {
      targetRecipes = categorySpecific;
    }
    // 만약 해당 카테고리가 200개 안에 하나도 없다면, 어쩔 수 없이 전체에서 추천 (또는 재시도 로직 가능)
  }
